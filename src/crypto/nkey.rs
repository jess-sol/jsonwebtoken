...
pub fn sign(key: &[u8], message: &[u8]) -> Result<String> {
    let signing_key = signature::Ed25519KeyPair::from_seed_unchecked(key)?;
    let out = signing_key.sign(message);
    Ok(b64_encode(out))
}

const ENCODED_SEED_LENGTH: usize = 58;
const PREFIX_BYTE_SEED: u8 = 0x90;

pub(crate) fn from_pubkey(key: &str) -> Result<[u8; 32]> {
    let raw = decode_raw(key.as_bytes()).map_err(|_| new_error(ErrorKind::InvalidSeedChecksum))?;
...
pub(crate) fn from_seed(source: &str) -> Result<(u8, [u8; 32])> {
    if source.len() != ENCODED_SEED_LENGTH {
        return Err(new_error(crate::errors::ErrorKind::InvalidSeedLength));
    }
    ...
    if b1 != PREFIX_BYTE_SEED {
        return Err(new_error(crate::errors::ErrorKind::InvalidSeedPrefix));
    }
...
fn decode_raw(raw: &[u8]) -> Result<Vec<u8>> {
    let mut b32_decoded = data_encoding::BASE32_NOPAD.decode(raw).unwrap();

    let checksum = crc::extract_crc(&mut b32_decoded);
    let v_checksum = crc::valid_checksum(&b32_decoded, checksum);
    if !v_checksum {
        Err(new_error(crate::errors::ErrorKind::InvalidSeedChecksum))
    } else {
        Ok(b32_decoded)
    }
}
